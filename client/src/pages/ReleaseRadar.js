...
<StyledParagraph>Its functionality hinges on Spotify’s ‘Release Radar’ playlist; an auto-generated, 30 track playlist that is curated based on a user's listening habits, followed artists, and music preferences. Each Friday the playlist is revised, nudging older tracks off to make way for new releases. The number of new releases varies, but seldom exceeds the full 30, therefore most playlists contain some tracks that have appeared on it before. </StyledParagraph>
 <StyledParagraph>This app first makes a get request to the ‘Release Radar’ playlist API to retrieve its most recent iteration. By cross referencing these tracks against those stored in a MongoDB database, it will then only display tracks that are new to the user. Presented with a list of new tracks, the user is then given the option to remove any track, or bulk add them to a playlist. Remove instructions are also stored in the database to ensure you are only asked once per track.</StyledParagraph>

<StyledParagraph>The user can then add all remaining tracks to a dedicated spotify playlist titled, for example, ‘Release Radar Capture Playlist July 2023’, which is itself generated by the app. If a playlist exists for the given month, the app will add the new tracks to that playlist. If it does not, it will create a new one, which over time will build a library of personalised playlists of new releases indexed by month and year, accessible both within your Spotify account and within the app.  </StyledParagraph>
</ReleaseRaderParaFlexBox>
<br/>
